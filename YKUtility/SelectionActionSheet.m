...
@interface SelectionActionSheet ()
...
@synthesize selectionActionSheetDelegate = _selectionActionSheetDelegate;
...
- (void)didPushDone:(id)sender {
    if (self.selectionActionSheetDelegate != nil) {
        [self.selectionActionSheetDelegate selectionActionSheet:self selectForRow:self.selectedRow];
    }
    [self dismissWithClickedButtonIndex:0 animated:YES];
}
...
- (NSString *)pickerView:(UIPickerView *)picker titleForRow:(NSInteger)row forComponent:(NSInteger)component {
	if (component == 0 && self.selectionActionSheetDelegate != nil) {
		return [self.selectionActionSheetDelegate selectionActionSheet:self titleForRow:row];
	} else {
		return nil;
	}
}
...
- (NSInteger)pickerView:(UIPickerView *)picker numberOfRowsInComponent:(NSInteger)component {
    if (self.selectionActionSheetDelegate != nil) {
        return [self.selectionActionSheetDelegate selectionActionSheetNumberOfRows:self];
    } else {
        return -1;
    }
}
